...
export function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    await redirectUserByRole(signInData.user.id);
    setIsLoading(false);
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: `${process.env.NEXT_PUBLIC_APP_URL}/auth/callback` }, // points to ne/ route.ts
    });
    if (error) {
  ...
  return (
    <form onSubmit={handleLogin} className="flex flex-col gap-6 p-8 rounded-3xl bg-gradient-to-br from-yellow-50 via-orange-50 to-pink-50 shadow-2xl dark:from-neutral-900 dark:via-neutral-800 dark:to-neutral-900">
      ...
      <div className="grid gap-4">
        ...
        <div className="grid gap-2">
          <div className="flex justify-between items-center">
            ...
            <Link href="/auth/forgot-password" className="text-sm underline text-yellow-500 hover:text-yellow-600">
              Forgot password?
            </Link>
          </div>
          <Input
        ...
        <Button variant="outline" className="w-full flex items-center justify-center gap-2" onClick={handleGoogleLogin} disabled={isLoading}>
