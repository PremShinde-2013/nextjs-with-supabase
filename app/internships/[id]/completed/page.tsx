  ...
  `)
    ...
    const drawCertificate = async (studentName: string, internshipName: string, verificationCode: string) => {
        ...
        return new Promise<string>(async (resolve, reject) => {
            try {
                ...
                const templateUrl =
                    "https://dhyeneqgxucokgtxiyaj.supabase.co/storage/v1/object/public/certificates/Internship_Certificate.png";
                ...
                template.onload = () => {
                    ...
                    ctx.textAlign = "left";
                    ctx.font = "700 65px 'Open Sans'";
                    ctx.fillText(studentName, 70, 630); // <-- adjust X,Y here
                    ctx.font = "700 48px 'Open Sans'";
                    ctx.fillText(internshipName, 70, 840); // <-- adjust X,Y here
                    ctx.font = "700 40px 'Open Sans'";
                    if (internshipDuration) {
                        ctx.font = "700 30px 'Open Sans'";
                        ctx.fillText(`${internshipDuration} Months`, 117, 937); // <-- adjust Y as needed
                    }
                    ctx.font = "600 40px 'Open Sans'";
                    ctx.fillText(new Date().toLocaleDateString(), 308, 1087);
                    ctx.textAlign = "center";
                    ctx.font = "700 30px 'Open Sans'";
                    ctx.fillText(`${userId?.slice(0, 6)}-${userInternshipId?.slice(0, 6)}`, 1662, 1020);
    ...
    const generateCertificate = async () => {
        ...
        try {
            ...
            const certificateUrl = `https://dhyeneqgxucokgtxiyaj.supabase.co/storage/v1/object/public/certificates/${fileName}`;
            ...
            {!certificate ? (
                <>
                    ...
                    {!reviewSubmitted ? (
                        ...
                        <>
                            <p className="mb-4 text-gray-600">✅ Review submitted! Now generate your certificate.</p>
                            <div className="bg-yellow-50 border border-yellow-300 rounded-lg p-4 mb-4">
                                <p className="text-sm text-yellow-800 font-medium">
                                    ⚠️ Disclaimer: You can generate your internship certificate only ONCE.
                            ...
                            {previewUrl && (
                                <div>
                                    ...
                                    <label htmlFor="confirm" className="text-sm text-gray-700">
                                        ⏳ Wait for Download Certificate button to be visible
                ...
                <div>
                    <h1 className="text-3xl font-extrabold mb-4 text-purple-700">🎉 Your Certificate is Ready!</h1>
                    <img
